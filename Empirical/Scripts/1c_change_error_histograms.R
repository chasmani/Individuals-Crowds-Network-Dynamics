...
columns_to_plot <- c('standardised_delta_crowd_error', 
                     'standardised_delta_s2',
                     'standardised_mean_delta_indy_error')

# Define custom labels for the subplots
custom_labels <- c(
  'standardised_delta_crowd_error' = "Change in Crowd Error",
  'standardised_delta_s2' = "Change in Variance",
  'standardised_mean_delta_indy_error' = "Change in Individual Error"
)
...
ggplot() +
  ...
  geom_text(data = percentages, 
            ...
            hjust = 0, vjust = 1.5, size = 5, color = color_improve, fontface="bold") +
  geom_text(data = percentages, 
            aes(x = 2.5, y = Inf, 
                label = sprintf("Above Zero: %.1f%%", above)),
            hjust = 1, vjust = 1.5, size = 5, color = color_worse, fontface="bold") +
...
ggsave("../Figures/delta_error_histograms_basic.png", dpi=600, bg="#ffffff")
