...
hlm_decent_improve = group_d %>% 
  ...
  lme(mu_ind_change ~ 1, random=~1|dataset,
                           data    = .)


hlmer_decent_improve = group_d %>% 
  subset(crowd_improvement == T) %>%
  subset(network=="decentralized") %>%
  lme4::lmer(formula=mu_ind_change ~ 1 + (1|dataset),
      data    = .)
...
summary(hlmer_decent_improve)
summary(hlm_decent_improve)
summary(lm_decent_improve)

aov(hlm_decent_improve) %>% summary
