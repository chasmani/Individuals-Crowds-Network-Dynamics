...
data_sum[,y_min := min(pct_not_worse) * 1.2, by = dataset]
data_sum[,y_max := 0 + abs(min(pct_not_worse)* 1.2), by = dataset]

data_sum %>%
  ggplot(
    aes(x=error_quartile, y=pct_not_worse, fill=network, group=network)) + 
