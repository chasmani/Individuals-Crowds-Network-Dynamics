...
data = out_d %>%
  subset(dataset!="becker 2020") %>% ## no repetition
  subset(network!="solo") %>%
  ungroup() %>%
  mutate(network=factor(network)) %>%
  group_by(dataset, network, group_id, skill_level) %>%
  summarise(
      group_id = unique(group_id)
    , skill = round(unique(skill_level))
...
data_sum = data %>% 
  group_by(dataset, network, skill) %>%
  summarySE(
    measurevar = "mu_ind_change"
    , groupvars = c("dataset", "network", "skill")
    , boot.trials = 1000
  )
...
data_sum %>%
  ...
  ggplot(
    aes(x=skill, y=mu_ind_change, fill=network, group=network)) + 
