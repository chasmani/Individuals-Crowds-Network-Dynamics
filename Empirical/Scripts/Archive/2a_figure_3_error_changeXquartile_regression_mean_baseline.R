...
source("1a_group_summaries_regression_mean_baseline.R")
...
data = out_d %>%
  ...
  summarise(
    ...
    , mu_ind_change_rand = mean(delta_err_rand)
...
data_comb = merge(
  data_sum
  , data_sum_rand[,c("network","dataset","error_quartile","mu_ind_change_rand")]
  , by=c("network","dataset","error_quartile")
)


data_comb %>%
  ggplot(
    aes(x=error_quartile, fill=network, group=network)) + 
    geom_bar(aes(y=mu_ind_change), stat = "identity", position = position_dodge(width = 0.9)) +
    geom_point(aes(y=mu_ind_change_rand), position = position_dodge(width = 0.9)) +
    geom_linerange(
...
ggsave(paste0("../Figures/New Figures/Figure 3/", "MEAN_ERROR_error_changeXquartile.png"), width = 6, height = 4, dpi = 1000)
