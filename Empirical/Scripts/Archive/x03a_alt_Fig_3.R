...
for(nets in unique(group_d$network)) {
  ...
  mod = group_d %>%
    ...
    group_by(dataset) %>%
    mutate(
      weight = (1 / length(pct_improve))/count_dat
    ) %>% 
    lm(
      pct_improve ~ 1 
      , weights = weight
...
ggsave("../Figures/New Figures/Figure 5/PCT_INDIVID_IMPROVED_X_main_effect_network.png", width = 4.5, height = 3.33, dpi=1000)
