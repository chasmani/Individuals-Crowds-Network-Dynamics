...
data_sum[,y_min := min(pct_indiv) * 1.2, by = dataset]
data_sum[,y_max := 0 + abs(min(pct_indiv)* 1.2), by = dataset]

data_sum %>%
  ggplot(
    aes(x=error_quartile, y=pct_indiv, fill=network, group=network)) + 
    geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
    geom_linerange(
      aes(ymax = pct_indiv+bootci, ymin= pct_indiv-bootci)
      , position = position_dodge(width = 0.9)
      ) +
  ggtitle("Percent Not Worse")+
  labs(x="Error Quartile",
       y="Percent of individuals not worse") +
  beckertheme +
...
ggsave(paste0("../Figures/New Figures/Figure 3/", "PCT_INDIVIDUAL_Xquartile.png"), width = 6, height = 4, dpi = 1000)
