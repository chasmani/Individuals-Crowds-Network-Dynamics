...
for k, z2 in enumerate(z2s):
	...
	h_boundary_d1 = 1/std_d2 *(std_e2 * cals - 2*beta_crowd_d1*std_e**2/cv)
	plt.plot(cals, h_boundary_d1, color=zero_color, linewidth=2, linestyle="dotted")  # Use a contrasting color
	...
	if k == 1:
		ax.set_xlabel(r"$- r(v, e^2)$")
	if k == 2:
		ax.set_xlabel("Calibration")

	# Set the y-axis label with rotated text
	if k == 0:
		ax.set_ylabel("Change in Crowd Error", rotation=90)	
	...
	if k == 1:
		ax.set_xlabel(r"$- r(v, d^2)$")
	if k == 2:
		ax.set_xlabel("Herding")

	# Set the y-axis label with rotated text
	if k == 0:
		ax.set_ylabel("Change in Average Individual Error", rotation=90)	
...
cbar_ax = fig.add_axes([0.85, 0.07, 0.05, 0.8])
...
plt.savefig("images/robust_benefits_cal_herd.png", dpi=600)
